...
var weather = {
    ...
    '(*extraInput) weather (*extraInput) tomorrow (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The forecast for tomorrow is ' + weather.forecast[1].text + ' with the maximum temperature being ' + weather.forecast[1].high + '°' + weather.units.temp + ' and the minimum temperature being ' + weather.forecast[1].low + '°' + weather.units.temp + '.');
            }
        });
    },

    '(*extraInput) tomorrow (*extraInput) weather (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
    ...
    '(*extraInput) forecast (*extraInput) tomorrow (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
    ...
    '(*extraInput) tomorrow (*extraInput) forecast (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
    ...
    '(*extraInput) weather (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('It is ' + weather.temp + '°' + weather.units.temp + ' and the forecast is ' + weather.currently + '. The maximum temperature is forecast to be ' + weather.high + '°' + weather.units.temp + ' and the minimum temperature is forecast to be ' + weather.low + '°' + weather.units.temp + '. The wind speed is ' + weather.wind.speed + weather.units.speed + ' towards ' + weather.wind.direction + ' and the humidity in the atmosphere is ' + weather.humidity + '%.');
            }
        });
    },

    '(*extraInput) forecast (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
    ...
    '(*extraInput) temperature (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The temperature is ' + weather.temp + '°' + weather.units.temp + ' and the forecast is ' + weather.currently + '.');
            }
        });
    },

    '(*extraInput) maximum (*extraInput) minimum (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The maximum temperature is forecast to be ' + weather.high + '°' + weather.units.temp + ' and the minimum temperature is forecast to be ' + weather.low + '°' + weather.units.temp + '.');
            }
        });
    },

    '(*extraInput) wind speed (*extraInput)': function() {
        weatherAPI.weather({
            wunderkey: '',
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The wind speed is ' + weather.wind.speed + weather.units.speed + ' towards ' + weather.wind.direction);
            }
        });
    },

    '(*extraInput) humidity (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The humidity in the atmosphere is' + weather.humidity + '%.');
            }
        });
    },

    '(*extraInput) sunrise and sunset (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The time of sunrise is ' + weather.sunrise + ' and the time for sunset is ' + weather.sunset);
            }
        });
    },

    '(*extraInput) sunset and sunrise (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The time of sunrise is ' + weather.sunrise + ' and the time of sunset is ' + weather.sunset);
            }
        });
    },

    '(*extraInput) sunrise (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The time of sunrise is ' + weather.sunrise);
            }
        });
    },

    '(*extraInput) sunset (*extraInput)': function() {
        weatherAPI.weather({
            location: 'Dehradun,UL',
            unit: 'C',
            success: function(weather) {
                athena.speak('The time of sunset is ' + weather.sunset);
            }
        });
    },
};
            ...
            if (e.readyState === b) {
                ...
                if (d.response) {
                    ...
                    g.title = f.item.title, g.temp = f.item.condition.temp, g.code = f.item.condition.code, g.todayCode = f.item.forecast[0].code, g.currently = f.item.condition.text, g.high = f.item.forecast[0].high, g.low = f.item.forecast[0].low, g.text = f.item.forecast[0].text, g.humidity = f.atmosphere.humidity, g.pressure = f.atmosphere.pressure, g.rising = f.atmosphere.rising, g.visibility = f.atmosphere.visibility, g.sunrise = f.astronomy.sunrise, g.sunset = f.astronomy.sunset, g.description = f.item.description, g.city = f.location.city, g.country = f.location.country, g.region = f.location.region, g.updated = f.item.pubDate, g.link = f.item.link, g.units = {
                        ...
                        temp: weatherAPI.getAltTemp(a.unit, f.item.condition.temp),
                        high: weatherAPI.getAltTemp(a.unit, f.item.forecast[0].high),
                        low: weatherAPI.getAltTemp(a.unit, f.item.forecast[0].low)
                    }, "f" === a.unit ? g.alt.unit = "c" : g.alt.unit = "f", g.forecast = [];
                    for (var j = 0; j < f.item.forecast.length; j++) h = f.item.forecast[j], h.alt = {
                        high: weatherAPI.getAltTemp(a.unit, f.item.forecast[j].high),
                        low: weatherAPI.getAltTemp(a.unit, f.item.forecast[j].low)
